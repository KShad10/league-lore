...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const config = await getPlayoffConfig(supabase, leagueId, parseInt(season))
    ...
    const matchups = await getPlayoffMatchups(supabase, leagueId, parseInt(season))
...
async function getPlayoffConfig(supabase: any, leagueId: string, season: number): Promise<PlayoffConfig> {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function getPlayoffMatchups(supabase: any, leagueId: string, season: number): Promise<MatchupData[]> {
  ...
  return (matchups || []).map(m => {
    ...
    return {
      ...
      team1Points: parseFloat(m.team1_points) || 0,
      team2Points: parseFloat(m.team2_points) || 0,
      winnerManagerId: m.winner_manager_id || '',
      team1Name: team1?.display_name || team1?.current_username || 'Unknown',
      team2Name: team2?.display_name || team2?.current_username || 'Unknown',
      winnerName: winner?.display_name || winner?.current_username || 'TBD',
...
function classifyMatchups(
  ...
  const lowerBracketEliminated = new Set<string>()
  ...
  for (const m of sortedMatchups) {
    ...
    if (isLowerBracket) {
      ...
      if (config.loserBracketType === 0) {
        lowerBracketEliminated.add(m.winnerManagerId)
      } else {
      ...
      matchupType = getPlaceGameType(roundNumber)
...
function getPlaceGameType(round: number): string {
